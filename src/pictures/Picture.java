...
public interface Picture {
	...
	default public Picture blur(int x0, int y0, int radius, int value) {
	...
	default public Picture blur(int ax, int ay, int bx, int by, int value) {
		for (int x = ax; x < bx; x++) {
			for (int y = ay; y < by; y++) {
				blur(x, y, value);
			}
	...
	default public Picture blur(int x, int y, int value) {
		double total_red = 0;
		double total_green = 0;
		double total_blue = 0;
		double total_pixels = 0;
		
		//Iterates through all surrounding pixels and calculates total r, g, b, and pixels
		for (int xoffset = -value; xoffset <= value; xoffset++) {
			for (int yoffset = -value; yoffset <= value; yoffset++) {

				try {
					Pixel p = getPixel(x + xoffset, y + yoffset);
					total_red += p.getRed();
					total_green += p.getGreen();
					total_blue += p.getBlue();
					total_pixels++;
				} catch (Exception e) {
				}
		...
		//Calculates the average pixel and paints the central pixel to reflect that
		try {
			Pixel avg_pixel = new ColorPixel(total_red / total_pixels, total_green / total_pixels,
